...
const BorderedTab = ({ children }: { children: ReactNode }) => (
  <div className="shrink-0 overflow-visible rounded-t-[5px] border-[1.5px] border-b-0 border-ink bg-tan px-3 py-1.5">
    {children}
  </div>
)

export const OrderSummaryHeader = () => (
  <div className="flex min-w-0 w-full items-end overflow-x-hidden overflow-y-visible">
    <BorderedTab>
      <h2 className="font-display text-lg font-bold uppercase leading-none tracking-wider text-ink md:text-xl">
        ORDER SUMMARY
      </h2>
    </BorderedTab>
...
const getOrderSummaryBodyClassName = (showOrderSummaryHeader: boolean) => {
  if (showOrderSummaryHeader) {
    return 'min-w-0 max-w-full space-y-2 pt-2'
  }
  return 'min-w-0 max-w-full space-y-2 pt-0'
}

const getWalletButtonClassName = (selected: boolean) => {
  const base =
    'relative flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-[10px] border-[1.5px] border-ink py-2.5'
  if (selected) {
    return `${base} bg-cream-warm`
  }
...
const CreditCardFields = ({ register, errors }: CreditCardFieldsProps) => (
  <div className="rounded-[10px] border-[1.5px] border-ink bg-tan p-2.5">
    <div className="mb-2 flex items-center gap-3">
      <RadioDot selected />
      <span className={cardBrandBox}>
        <RiVisaLine className="h-4 w-8 text-[#1a1f71]" />
      </span>
      <span className={cardBrandBox}>
        <SiMastercard className="h-4 w-8 text-[#eb001b]" />
      </span>
    </div>
...
export const DesktopCheckout = ({
  ...
  const orderSummaryBodyClassName = getOrderSummaryBodyClassName(showOrderSummaryHeader)
  const appleButtonClassName = getWalletButtonClassName(isApple)
  const bankButtonClassName = getWalletButtonClassName(isBank)

  const totalRows = totals.map(({ label: lbl, value }) => (
    <div key={lbl} className="flex max-w-full items-center justify-end gap-2">
      ...
      <span className="text-right font-display text-sm font-medium text-ink">{formatMoney(value)}</span>
    </div>
  ))

  let orderSummaryHeader = null
  if (showOrderSummaryHeader) {
    orderSummaryHeader = <OrderSummaryHeader />
  }

  let creditCardBlock = null
  if (isCard) {
    creditCardBlock = <CreditCardFields register={register} errors={errors} />
  }

  const handleSelectCard = () => {
    onPaymentChange('credit-card')
  }

  const handleSelectPaypal = () => {
    onPaymentChange('paypal')
  }

  const handleSelectApple = () => {
    onPaymentChange('apple-pay')
  }

  const handleSelectBank = () => {
    onPaymentChange('bank-transfer')
  }
  ...
  return (
    <div className="flex w-full min-w-0 max-w-full flex-col gap-2.5">
      <section className="min-w-0 max-w-full">
        ...
        <div className={orderSummaryBodyClassName}>
          ...
          <div>
            ...
            <div className="mt-4 border-b-[1.5px] border-ink" />
          </div>

          <div className="flex min-w-0 max-w-full flex-col items-end gap-1.5 py-1">{totalRows}</div>
        </div>
      </section>

      <section className="min-w-0 max-w-full">
        <div className="inline-block rounded-[5px] border-[1.5px] border-ink bg-tan px-3 py-1">
        ...
        <div className="mt-2 space-y-2">
          <div className="flex items-center gap-8">
            <button type="button" onClick={handleSelectCard} className="flex items-center gap-2.5">
              <RadioDot selected={isCard} />
              <span className="font-display text-xs font-medium uppercase text-ink">CREDIT/DEBIT CARD</span>
            </button>
            <button type="button" onClick={handleSelectPaypal} className="flex items-center gap-2.5">
          ...
          <div className="flex min-w-0 gap-2">
            <button type="button" onClick={handleSelectApple} className={appleButtonClassName}>
            ...
            <button type="button" onClick={handleSelectBank} className={bankButtonClassName}>
              <div className="absolute left-3 top-3">
